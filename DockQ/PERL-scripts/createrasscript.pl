...
if (scalar(@ARGV) == 0)
{
print STDERR "This PERL-script generates the interfacial contact network for the native (default)\n";
print STDERR "It assumes that ./DockQ.exe has been run on a pair of files and fort.23 (interface contact file) has been generted in the working directory\n";
print STDERR "usage: PERL-scripts-for-alignment-and-renumbering-of-PDB-files/createrasscript.pl <native.pdb>\n\n";
exit;
}

$pdb = $ARGV[0] || die "Enter native PDB file as cmd\n";
$scon = 'fort.23';
...
open (INP,"<$scon") || die "fort.23 not found in the current directory\nrun: ./DockQ.exe <model.pdb> <native.pdb> -ras 1\n\n";
@dat = <INP>;
open (PDB,"<$pdb") || die "$pdb not found in the current directory\n";     # mapped according to .scon;
$snet = $scon;

$snet = $pdb.'.raspdb';
$scon_renamed = $pdb.'.scon';
`mv $scon $scon_renamed`;
$scon=$scon_renamed;
